...
const permissionResolvers = {
  Query: {
    ...
    permission: async (_, { id }) => {
      return await permissionRepo.findOne({
        where: { id },
        relations: ["roles"],
      });
    },
  ...
  Mutation: {
    ...
    createPermission: async (_, { input }) => {
      ...
      if (!name || !slug || !permission_group) {
        throw new Error("Name, slug, and permission_group are required.");
      }
    ...
    deletePermission: async (_, { id }) => {
      ...
      if (result.affected === 0) {
        throw new Error(`Permission with ID ${id} not found or already deleted.`);
      }
